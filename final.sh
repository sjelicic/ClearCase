...
if [ "$#" != "2" ]; then
  echo ""
  echo "Usage:          ./cc_git_script.sh [integration stream] [development stream]"
  echo "Example:        ./cc_git_script.sh Linux_R6000_ints GCSS_Linux_R6000_ints"
  echo ""
  #exit 300
fi
...
###Binary file identification, not tested
#Using Pearl
#find . -type f -not -empty | perl -lne 'print if -B' | xargs git lfs track {}

#git lfs is not installed, not tested
...
#until BinaryCheck
#do
#       echo "re-running the binary file check"
#done

#Using file utility, not tested
